...
export default function ChannelClient({ channelId, isSubscribed }: { channelId: string; isSubscribed: boolean }) {
  ...
  useEffect(() => {
    document.body.style.overflow = (activeVideo || activePlaylist || activePlaylistVideo) ? 'hidden' : ''
    return () => { document.body.style.overflow = '' }
  }, [activeVideo, activePlaylist, activePlaylistVideo])
  ...
  // ── Shared video sheet content ─────────────────────────────────────────────
  const VideoSheetBody = ({
    videoId, title, publishedAt, description, onClose,
    playlistContext, isSubscribed,
  }: {
    videoId: string; title: string; publishedAt: string; description: string
    onClose: () => void; playlistContext?: { items: PlaylistItem[]; currentIndex: number }; isSubscribed: boolean
  }) => (
    <div style={{ padding: '0 16px' }}>
      ...
      {/* Channel row – conditional subscribe */}
  ...
  // How tall is the bottom nav — used for all sheet positioning
  const NAV_H = 72
    ...
    <>
      <style>{`
        ...
        * { box-sizing: border-box; }
        ...
        /* ── FIX: sheets start from above the bottom nav (${NAV_H}px) ── */
        @keyframes slideUp{
          from { transform: translateX(-50%) translateY(calc(100% + ${NAV_H}px)); }
        ...
        /* ── FIXED: bottom sits above nav, max-height fills remaining space ── */
        .sheet{
          position:fixed;left:50%;transform:translateX(-50%);
          bottom:${NAV_H}px;
          width:100%;max-width:390px;
          background:#0d0f1a;border-radius:24px 24px 0 0;
          border-top:1px solid rgba(255,255,255,0.08);
          z-index:101;
          animation:slideUp 0.3s cubic-bezier(0.32,0.72,0,1);
          max-height:calc(100vh - ${NAV_H}px - 20px);
          overflow-y:auto;scrollbar-width:none;
        }
        .sheet::-webkit-scrollbar{display:none}

        /* ── FIXED: same for the stacked playlist-video sheet ── */
        .sheet-top{
          position:fixed;left:50%;transform:translateX(-50%);
          bottom:${NAV_H}px;
          width:100%;max-width:390px;
      ...
      <div style={{ background: '#080b12', minHeight: '100vh', position: 'relative' }}>
        ...
        <div className="ch-root" style={{ zIndex: 1, position: 'relative', paddingBottom: 100 }}>
          ...
          <div style={{ padding: '0 20px', marginTop: -42, position: 'relative', zIndex: 2 }}>
            ...
            {/* Subscribe button group – conditional */}
                ...
                <div style={{ padding: '0 20px' }}>
                  {[1, 2, 3, 4].map(i => (
                    <div key={i} style={{ display: 'flex', gap: 12, padding: '12px 0', alignItems: 'center' }}>
                      <div className="skeleton" style={{ width: 120, height: 67, borderRadius: 10, flexShrink: 0 }} />
                      <div style={{ flex: 1 }}>
