    ...
    <>
      <style>{`
        ...
        * { box-sizing: border-box; }
        .gd-root { background: #080b12; min-height: 100vh; color: white; font-family: 'Syne', sans-serif; max-width: 390px; margin: 0 auto; position: relative; }
        .card-glass {
        ...
        .goal-row:active { transform: scale(0.98); }
        @keyframes fadeSlideUp {
          from { opacity: 0; transform: translateY(10px); }
          to   { opacity: 1; transform: translateY(0); }
        }
        .ani { animation: fadeSlideUp 0.3s ease forwards; }
      `}</style>
      ...
      <div className="gd-root">
        ...
        <div style={{ position: 'relative', zIndex: 1, overflowY: 'auto', overflowX: 'hidden', height: '100vh', paddingBottom: 120 }}>
            ...
            <div style={{
              ...
              <div style={{ position: 'relative', zIndex: 1 }}>
                ...
                <div style={{ display: 'flex', gap: 10 }}>
                  {[
                    { icon: <Users size={13} color="#a855f7" />, val: membersWithProfiles.length, label: 'members', bg: 'rgba(168,85,247,0.12)', border: 'rgba(168,85,247,0.2)' },
                    { icon: <Target size={13} color="#06b6d4" />, val: goals?.length ?? 0, label: 'goals', bg: 'rgba(6,182,212,0.12)', border: 'rgba(6,182,212,0.2)' },
                  ].map(s => (
                    <div key={s.label} style={{ background: s.bg, border: `1px solid ${s.border}`, borderRadius: 12, padding: '8px 14px', display: 'flex', alignItems: 'center', gap: 6 }}>
                      {s.icon}
                  ...
                  <div style={{ background: group.is_public ? 'rgba(16,185,129,0.12)' : 'rgba(245,158,11,0.12)', border: `1px solid ${group.is_public ? 'rgba(16,185,129,0.2)' : 'rgba(245,158,11,0.2)'}`, borderRadius: 12, padding: '8px 14px', display: 'flex', alignItems: 'center', gap: 5 }}>
                    {group.is_public ? <Globe size={13} color="#10b981" /> : <Lock size={13} color="#f59e0b" />}
                    <span style={{ fontSize: 11, color: group.is_public ? '#10b981' : '#f59e0b', fontWeight: 600 }}>{group.is_public ? 'Public' : 'Private'}</span>
                  </div>
                </div>
              </div>
            </div>
          ...
          {/* Goals */}
          {isMember && (
            <div style={{ padding: '24px 20px 0' }} className="ani">
